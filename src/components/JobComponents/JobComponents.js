...
class JobComponents extends Component {
  constructor(props)
    {
        super(props)
    ...
    jobIDValidator = () => {
      ...
      {
        fetch('http://localhost:3600/api/v1/jobs/validate',{
                method:'POST',
                crossDomain: true,
                headers: {
    ...
    formSubmitHandler = (event) =>{
        ...
        fetch('http://localhost:3600/api/v1/jobs',{
        method:'PATCH',
        crossDomain: true,
        headers: {
            'Content-type':'application/json',
  ...
  render() {
    ...
    return (
        <form className='form-container' onSubmit={this.formSubmitHandler}>
            <h2>Adding a new Employee Details</h2>
